...
/*   1. Массив скиллов (поле skills) всех людей, не должно быть повторяющихся скиллов, 
    ...
    var arrSkills = _.map(arrObj, 'skills');
    $('body').append('skills array:',
        _.sortBy(_.uniq(_.flattenDeep(arrSkills)), function (i) { return i.toLowerCase(); } ) );
    ...
    $('body').append('name array:',
        _.map(_.sortBy(arrObj, function (o) { return o.friends.length; }), 'name'));
    ...
    $('body').append('friends array:',
        _.uniq(_.map(_.flattenDeep(_.map(arrObj, 'friends')), 'name')));
